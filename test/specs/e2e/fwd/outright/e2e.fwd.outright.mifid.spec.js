...
describe('E2E scenario for Forward outright', () => {
  it('FWDOR_06_C12521 FWD USD HKD 1W TEFD (New)', () => {
    ...
    deal.placeOutrightOrder(instrument.CURRENCY_A, instrument.CURRENCY_Q, '', instrument.TENOR_B, '7.79395', '15');
    ...
    log.log(`Test Case Number : FWDOR_06_C12521 and Deal ID : ${dealId}`);
    e2e.saveDealTestMap(1, `Test Case Number : FWDOR_06_C12521 and Deal ID : ${dealId}`);
    e2e.saveDealIdWithTest(`${dealId}@@complete:new:leg1`, `FWDOR_06_C12521@@FWD=>fwd_outright=>mifid:FWD_USD_HKD_1W_${instrument.VENUE_E}_NEW`);
    popUpNav.closePopUpMessage();
  }).timeout(30000);

  it('FWDOR_07_C12522 FWD USD CNH 2M TEMM (New)', () => {
    expect(login.getDdlUserDropdownText()).to.equal(users.USER_A.FullName);
    strategy.clickRdoStrategyOutright();
    venue.selectExecutionVenue(instrument.VENUE_G);
    settings.ratesFeedOff();
    deal.placeOutrightOrder(instrument.CURRENCY_A, instrument.CURRENCY_S, '', instrument.TENOR_E1, '7.01105', '15');
    clientTrader.selectBuyerTrader(users.CLIENT_A.Client, users.TRADER_A);
    clientTrader.selectSellerTrader(users.CLIENT_B.Client, users.TRADER_B);
    broker.selectBuyerBrokerName(users.DESK_F, users.BROKER_L);
    broker.selectSellerBrokerName(users.DESK_F, users.BROKER_M);
    deal.clickSubmitBtn();
    const dealId = e2e.getDealId();
    log.log(`Test Case Number : FWDOR_07_C12522 and Deal ID : ${dealId}`);
    e2e.saveDealTestMap(1, `Test Case Number : FWDOR_07_C12522 and Deal ID : ${dealId}`);
    e2e.saveDealIdWithTest(`${dealId}@@complete:new:leg1`, `FWDOR_07_C12522@@FWD=>fwd_outright=>mifid:FWD_USD_CNH_2M_${instrument.VENUE_G}_NEW`);
    popUpNav.closePopUpMessage();
  }).timeout(30000);

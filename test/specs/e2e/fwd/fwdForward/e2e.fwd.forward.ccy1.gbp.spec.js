...
describe('E2E scenario for Forward Forward CCY GBP', () => {
  it('FWD_FWD_09_C12515 FWD GBP USD 6M*9M XOFF (New)', () => {
    expect(login.getDdlUserDropdownText()).to.equal(users.USER_A.FullName);
    strategy.strategy.clickRdoStrategyFwdForward();
    venue.selectExecutionVenue(instrument.VENUE_B);
    settings.ratesFeedOff();
    deal.placeFwdForwardOrderDetails(instrument.CURRENCY_P, instrument.CURRENCY_A, '', instrument.TENOR_I, '1.3116997', '0.0031', '1', instrument.TENOR_I1, '', '1');
    ...
    log.log(`Test Case Number : FWD_FWD_09_C12515 and Deal ID : ${dealId}`);
    e2e.saveDealTestMap(1, `Test Case Number : FWD_FWD_09_C12515 and Deal ID : ${dealId}`);
    e2e.saveDealIdWithTest(`${dealId}@@complete:new:leg1`, `FWD_FWD_09_C12515@@FWD=>fwd_fwd=>gbp_ccy:FWD_GBP_USD_6M_9M_${instrument.VENUE_B}_NEW`);

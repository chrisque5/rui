...
describe('E2E scenario for Forward third party agents ', () => {
  it('FWD_07_C12500 FWD USD THB 1M XOFF (New)', () => {
    ...
    log.log(`Test Case Number : FWD_07_C12500 and Deal ID : ${dealId}`);
    e2e.saveDealTestMap(1, `Test Case Number : FWD_07_C12500 and Deal ID : ${dealId}`);
    e2e.saveDealIdWithTest(`${dealId}@@complete:new:leg1`, `FWD_07_C12500@@FWD=>fwd=>agents:FWD_USD_THB_1M_${instrument.VENUE_B}_NEW`);
    popUpNav.closePopUpMessage();
  }).timeout(30000);

  it('FWD_08_C12501 FWD USD CNH 1M XOFF (New)', () => {
    expect(login.getDdlUserDropdownText()).to.equal(users.USER_A.FullName);
    strategy.clickRdoStrategyForward();
    venue.selectExecutionVenue(instrument.VENUE_B);
    settings.ratesFeedOff();
    deal.placeForwardOrderDetails(instrument.CURRENCY_A, instrument.CURRENCY_S, '', instrument.TENOR_E, '7.00265', '0.006483', '10', '', '10');
    ...
    deal.clickSubmitBtn();
    const dealId = e2e.getDealId();
    log.log(`Test Case Number : FWD_08_C12501 and Deal ID : ${dealId}`);
    e2e.saveDealTestMap(1, `Test Case Number : FWD_08_C12501 and Deal ID : ${dealId}`);
    sharedStore.setValueToStore('C12501', dealId);
    e2e.saveDealIdWithTest(`${dealId}@@complete:new:leg1`, `FWD_08_C12501@@FWD=>fwd=>agents:FWD_USD_CNH_1M_${instrument.VENUE_B}_NEW`);
    popUpNav.closePopUpMessage();
  }).timeout(30000);
